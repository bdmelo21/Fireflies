...
function updateCanvas() {
  ...
  currentGame.player.drawPlayer();
  hitBottom();
  currentGame.player.vy =
    currentGame.player.vy +
    (currentGame.player.gravity - currentGame.player.userPull);
...
function isOnFirstPlaform() {
  if (
    currentGame.player.y > 480 &&
    currentGame.player.y < 550 &&
    currentGame.player.x > 860 &&
    currentGame.player.x < 1060
  ) {
    currentGame.player.y = 490;
    currentGame.player.vy = 0;
    currentGame.player.jumping = false;
  } else if (
    currentGame.player.y > 290 &&
    currentGame.player.y < 350 &&
    currentGame.player.x > 440 &&
    currentGame.player.x < 710
  ) {
    currentGame.player.y = 300;
    currentGame.player.vy = 0;
    currentGame.player.jumping = false;
  } else if (
    currentGame.player.y > 110 &&
    currentGame.player.y < 140 &&
    currentGame.player.x > 60 &&
    currentGame.player.x < 300
  ) {
...
function firstLevel() {
  ...
  let pink = new Image();
  pink.src = "images/FCharacter.png";
...
function secondLevel() {
  ...
  let pink = new Image();
  pink.src = "images/FCharacterLeft.png";
...
function isPlatforms2() {
  balooncounter = true;
  if (
    currentGame.player.y > 68 &&
    currentGame.player.y < 132 &&
    ...
    currentGame.player.y > 580 &&
    currentGame.player.y < 652 &&
    ...
    switch (damagecounter) {
      case true:
        ...
        document.getElementsByClassName(
          "lifes"
        )[0].innerHTML = `x${currentGame.lifes}`;
        break;
    }
  } else if (
    currentGame.player.y > 580 &&
    currentGame.player.y < 652 &&
    currentGame.player.x > 430 &&
    currentGame.player.x < 620
  ) {
    ...
    if (currentGame.player.counter === 2) {
      ...
      setTimeout(() => {
        ...
        backgroundlast.style.background =
          "url('videos/thankyou.gif') no-repeat center";
